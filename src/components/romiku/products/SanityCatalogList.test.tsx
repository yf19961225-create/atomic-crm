...
const product = (overrides: Partial<SanityCatalogProduct> = {}): SanityCatalogProduct => ({
  id: "sanity-1",
  skuSort: "RMK-100",
  isPublished: true,
  sku: "RMK-100",
...
describe("SanityCatalogList", () => {
  ...
  it("renders the published Sanity product fields returned for the catalog page", async () => {
    ...
    await expect.element(screen.getByRole("cell", { name: "RMK-100" })).toBeVisible();
    await expect.element(screen.getByRole("cell", { name: "收纳盒" })).toBeVisible();
    await expect.element(screen.getByRole("cell", { name: "Storage box" })).toBeVisible();
    await expect.element(screen.getByRole("cell", { name: "120 个" })).toBeVisible();
    await expect.element(screen.getByRole("cell", { name: "彩盒" })).toBeVisible();
    await expect.element(screen.getByRole("cell", { name: "24" })).toBeVisible();
  });

  it("keeps the catalog readable when optional Sanity fields are missing", async () => {
    getPage.mockResolvedValue({
      products: [product({ sku: undefined, name: { en: "English name" }, category: undefined, moqQuantity: undefined, moqUnit: undefined, packaging: undefined, cartonQty: undefined })],
    });

    const screen = await render(<SanityCatalogList />);

    await expect.element(screen.getByRole("cell", { name: "English name" }).first()).toBeVisible();
