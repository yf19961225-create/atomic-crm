import { romikuSanityConfig } from "./romikuSanityProductSource";
...
export type SanityCatalogProduct = Omit<SanityCatalogRecord, "_id"> & { id: string; skuSort: string; isPublished: boolean };
export type CatalogFilter = { search: string; includeUnpublished: boolean; after?: { skuSort: string; id: string } };
...
export const buildCatalogQuery = (filter: CatalogFilter) => {
  ...
  return {
    ...
    params: { search: `*${filter.search}*`, afterSku: filter.after?.skuSort ?? "", afterId: filter.after?.id ?? "" },
    limit: catalogPageSize + 1,
  };
};

const map = (record: SanityCatalogRecord): SanityCatalogProduct => ({ ...record, id: record._id, skuSort: record.sku ?? "", isPublished: record.isPublished === true });
export const mapCatalogPage = (records: SanityCatalogRecord[], size = catalogPageSize) => {
  const products = records.slice(0, size).map(map);
  const last = products.at(-1);
  return { products, nextCursor: records.length > size && last ? { skuSort: last.skuSort, id: last.id } : undefined };
};

export const createSanityCatalogSource = () => ({
  async getPage(filter: CatalogFilter) {
    const built = buildCatalogQuery(filter);
    ...
    const params = new URLSearchParams({ query: built.query, $search: JSON.stringify(built.params.search), $afterSku: JSON.stringify(built.params.afterSku), $afterId: JSON.stringify(built.params.afterId), $limit: String(built.limit) });
