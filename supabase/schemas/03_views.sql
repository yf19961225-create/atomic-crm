...
create or replace view public.romiku_formal_customer_directory with (security_invoker = true) as
select customer.id,customer.name,customer.country,customer.logistics,
  outbound.brand_name as brand,outbound.city,contact.name as contact,contact.email,contact.phone,contact.whatsapp,
...
left join lateral (
  select name,email,phone,whatsapp from public.romiku_customer_contacts
