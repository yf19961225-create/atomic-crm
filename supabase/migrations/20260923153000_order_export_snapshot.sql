...
begin
  if not (coalesce(new.terms_snapshot, '{}'::jsonb) ? 'order_export') then
    new.terms_snapshot := coalesce(new.terms_snapshot, '{}'::jsonb) || jsonb_build_object(
      'order_export', jsonb_build_object(
        'template_key', 'order',
        'seller', jsonb_build_object('company_name','YIWU ROMIKU NAIL SUPPLY 义乌络洣库美甲','address','No. 1, Qingkou East Road, Yiwu, Zhejiang, China','tel_whatsapp','+86 190 2577 7589','website','www.romiku.com','email','info@romiku.com'),
